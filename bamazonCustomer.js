...
var makeTable = function() {
  ...
  connection.query("SELECT * FROM products", function(err, res) {
    ...
    var tab = "\t";
    console.table("ItemID\tProduct Name\tDepartment Name\tPrice\t# In Stock");
    console.log("--------------------------------------------------------");

    // For loop goes through the MySQL table and prints each individual row on a new line
    for (var i = 0; i < res.length; i++) {
      console.table(res[i].item_id + tab + res[i].product_name + tab +
          ...
          if ((res[id].stock_quantity - val.quant) > 0) {
            ...
            connection.query(
              ...
              function(err, res2) {
                ...
                console.log("\n  " + val.quant + " of " + product + " bought for " + (res[id].price * val.quant) + "\n");
